...
export default function Sidebar({
  ...
  const handleGenerate = async () => {
    ...
    const bearerToken = "U2NOA00V0R67H4N6YJAK5J5CCMGS22YONRWZ4YNS";
    ...
    if (model === "Brush") {
      const brushReferenceImages = images.filter(
        (image) => image.type === "reference"
      );
      if (!baseImage) {
      ...
      maskedImageURLs = await Promise.all(
        maskImages.map(async (imageData, index) => {
          ...
          const maskedImageFile = new File(
            [maskedImageDataBlob],
            `masked_image_${timestamp}_${index}.png`
          );
    ...
    switch (model) {
      ...
      case "Light_Simple":
        ...
        break;
      case "Upscale_Detail":
        const referenceImage = newImages.find(
          (img) => img.type === "reference"
        )?.url;
        payload = {
          input: {
  ...
  const pollForStatus = async (jobId) => {
    const apiStatusEndpoint = `https://api.runpod.ai/v2/t823dessgfsjuh/status/${jobId}`;
    const bearerToken = "U2NOA00V0R67H4N6YJAK5J5CCMGS22YONRWZ4YNS";
                  ...
                  <label className="w-full mb-2 cursor-pointer block relative flex flex-col items-center">
                    ...
                    <span className="mt-2 text-sm">
                      {image.type === "base"
                        ...
                        : `Image Ref # ${
                            model === "Brush" ? referenceIndex + 1 : index - 1
                          }`}
                    </span>
                  </label>
                  {image.type !== "base" &&
                ...
                {model === "Brush" && image.type === "reference" && (
                  <>
                    <Slider
                      label="Strength"
                      value={
                        sliderValues.reference[referenceIndex]?.strength || 0
                      }
                      min={0.8}
                      max={1.3}
                      initialValue={1}
      ...
      {model == "Pencil" && (
        <>
          ...
          <div className="mb-4">
            ...
            <div className="bg-white p-2 rounded-2xl shadow mb-4 border border-2 border-black flex items-center justify-center">
              <div
                className={`w-full bg-customBG1 flex items-center rounded-lg justify-center ${
                  !edgesImage ? "p-20" : ""
                }`}
              >
                {edgesImage ? (
